...
class CustomDataset:
    ...
    def get_image_relpaths(self, i: Union[int, List[int]]) -> Union[List[str], str]:
        """
        ...
        if hasattr(self, "_imgs_level"):
            s = self._imgs_level
        rel_paths = ["/".join(img_paths[k].split("/")[-s:]) for k in indices]
        if type(i) == int:
            return rel_paths[0]
        return rel_paths
...
def to_pil_list(x) -> List[Image.Image]:
    ...
    - imgs_pil:     A list of PIL Images (length is the number of
                    images in 'x')
    """
    if type(x) == Image.Image or (type(x) == list and type(x[0]) == Image.Image):
    ...
    def __init__(
        ...
        if self.facet == "token":
            self.hook_handle = self.model.encoder.layer[
                self.layer
            ].register_forward_hook(self._generate_forward_hook(self.facet))
        elif self.facet == "key":
            self.hook_handle = self.model.encoder.layer[
                self.layer
...
def pad_img(img: np.ndarray, padding: int, color: tuple = (0, 0, 0)) -> np.ndarray:
    ...
    #                         (color, color), (0, 0)])
    ret_img = (
        np.ones((img.shape[0] + 2 * padding, img.shape[1] + 2 * padding, 3), np.uint8)
        * color
    )
...
def reduce_pca(
    ...
    if low_factor == 0.0:  # Direct downsample
        ...
        pca.fit(train_descs)
        tf_pca = np.concatenate((pca.components_[:_up], pca.components_[-_down:]))
...
def concat_desc_dists_clusters(
    cluster_centers: torch.Tensor, descs: torch.Tensor
) -> torch.Tensor:
    """
